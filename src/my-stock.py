...
def stocks_data():
    ...
    stocks = stock_zh_a_spot_em_df[stock_zh_a_spot_em_df['代码'].isin([str(i) for i in stocks_info()[1]])].iloc[:,
             [1, 2, 3, 4, 5]]

    for _, row in stocks.iterrows():
        _data.append([row['名称'], row['最新价'], row['涨跌幅']])
    return _data
...
class MainWindow(QMainWindow):
    ...
    def update_table(self, _stocks_data):
        for i in range(self.rows):
            item = QTableWidgetItem(_stocks_data[i][0])
